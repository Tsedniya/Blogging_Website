...
const Sidebar = ({ onMenuClick }) => {
  ...
  const bottomItems = [
    ...
    { id: "notifications", label: "Notifications", icon: <FiBell size={20} /> },
    {
      ...
      onClick: () => {
        ...
        localStorage.removeItem("laravel-token"); // Remove token from localStorage
  ...
  const renderMenuItem = (item) => (
    <li
      ...
      onClick={() => {
        if (item.onClick) {
          item.onClick(); // Call the custom onClick function if it exists
        } else if (item.link) {
      ...
      ${
        active === item.id
          ? "bg-blue-800 text-black font-semibold"
          : "hover:bg-gray-400 text-gray-300 font-bold"
      }`}
    >
      {item.icon}
  ...
  return (
    <aside className="w-72 h-screen  text-black flex flex-col justify-between fixed left-0 top-0 overflow-y-auto shadow-xl border-r border-gray-700">
      ...
      {/* Navigation Section */}
      <nav className="mt-4">
        <ul className="space-y-3">{topItems.map(renderMenuItem)}</ul>
      </nav>

      {/* Bottom Section */}
      <div className="mt-auto mb-6">
        <ul className="space-y-3">{bottomItems.map(renderMenuItem)}</ul>
      </div>
    </aside>
  );
};
